...
exports.run = (client, message, args, tools) => {
  ...
  if (!channel) return tools.error(message, 'Mention a channel. Usage: `setiwlc #channel`', client);

  const db = require(dbPath);
  if (!db[message.guild.id]) db[message.guild.id] = {};

  db[message.guild.id].iwlcchannel = channel.id;
  db[message.guild.id].iwlctoggle  = true;
  save(db);

  const embed = tools.brandedEmbed(client, 'success')
    .setTitle(':frame_photo: Image Welcome Channel Set')
    .addFields({ name: 'Channel', value: `${channel}` });
  message.channel.send({ embeds: [embed] });
};

exports.help = { name: 'setiwlc', description: 'Set image welcome channel.', usage: 'setiwlc #channel' };